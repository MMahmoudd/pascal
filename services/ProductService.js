...
export default {
    getAllProducts(category, minValue, maxValue) {
        ...
        return Service.get(`${resource}/items?restaurant_id=6&${filterQuery}`)
            .then((response) => {
    ...
    getAllTopItems() {
        return Service.get(`${resource}/topItems?restaurant_id=6`)
            .then((response) => {
                if (response.status === 200) {
    ...
    getProductById(id) {
        return Service.get(`${resource}/items/${id}?restaurant_id=6`)
            .then((response) => {
                if (response.status === 200) {
                    return response.data
                }
    ...
    getsearchedProducts(keyword) {
        ...
        return Service.get(`${resource}/search_items?restaurant_id=6&${filterQuery}`)
            .then((response) => {
                if (response.status === 200) {
                    if (response.data.items.length > 0) {
                        response.data.items.map((item) => {
