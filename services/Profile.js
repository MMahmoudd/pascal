...
export default {
    ...
    getUserAddress() {
        return Service.get(`${resource}/my_addresses?restaurant_id=6`, {
                headers: {
                    Authorization: "Bearer " + localStorage.getItem("token"),
                },
            })
    ...
    getGovernrate() {
        return Service.get(`${resource}/governrates?restaurant_id=6`, {
            headers: {
                Authorization: "Bearer " + localStorage.getItem("token"),
            },
        }).then((response) => {
    ...
    getAreas(id) {
        return Service.get(
            `${resource}/areas?restaurant_id=6&governrate_id=${id}`, {
                headers: {
                    Authorization: "Bearer " + localStorage.getItem("token"),
                },
            }
